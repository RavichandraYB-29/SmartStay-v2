...
This project is designed so that each developer uses their own backend. The following files are gitignored and must be created locally by each developer:

| File | How to Get It |
|------|---------------|
| `lib/firebase_options.dart` | Run: `flutterfire configure` |
| `lib/config/env.dart` | Copy from `env.dart.example` and fill in |
| `android/app/google-services.json` | Generated by `flutterfire configure` |
| `.firebaserc` | Copy from `.firebaserc.example` and fill in |
| `firebase.json` | Copy from `firebase.json.example` and fill in |

Your Firebase project and PayU account are completely separate from the original author's. Your data stays in your own Firebase project.
...
> ⚠️ **Important**: Do NOT copy `firebase_options.dart` from this repo. You must generate your own using `flutterfire configure` with your own Firebase project. See [SETUP.md](SETUP.md) for full instructions.
